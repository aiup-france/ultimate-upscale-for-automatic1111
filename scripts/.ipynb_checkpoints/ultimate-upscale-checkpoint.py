...
from core.ComfyUI_UltimateSDUpscale.modules import processing, shared, images, devices, scripts
from core.ComfyUI_UltimateSDUpscale.modules.processing import StableDiffusionProcessing
from core.ComfyUI_UltimateSDUpscale.modules.processing import Processed
from core.ComfyUI_UltimateSDUpscale.modules.shared import opts, state
...
class USDUpscaler():
    ...
    def upscale(self):
        ...
        if self.upscaler.name == "None":
            ...
            print("-1self image upsacle", self.image.mode)
            return
        ...
        for index, value in self.scales:
            ...
            print("_before2self image upsacle", self.image.mode)
            print("value", value)
            print("self.upscaler.data_path",self.upscaler.data_path)
            self.image = self.upscaler.scaler.upscale(self.image, value, self.upscaler.data_path)
            print("_2self image upsacle", self.image.mode)
        # Resize image to set values
        self.image = self.image.resize((self.p.width, self.p.height), resample=Image.LANCZOS)
        print("-3self image upsacle", self.image.mode)
    ...
    def save_image(self):
        if type(self.p.prompt) != list:
            ...
            print("2self image upsacle", self.image.mode)
        else:
    ...
    def process(self):
        ...
        if self.redraw.enabled:
            print("process--------", self.image.mode)
            self.image = self.redraw.start(self.p, self.image, self.rows, self.cols)
            print("4self image upsacle", self.image.mode)
        ...
        if self.seams_fix.enabled:
            ...
            print("5self image upsacle", self.image.mode)
...
class Script(scripts.Script):
    ...
    def run(self, p, _, tile_width, tile_height, mask_blur, padding, seams_fix_width, seams_fix_denoise, seams_fix_padding,
        ...
        seams_fix_denoise=1.0
        seams_fix_mask_blur=8
        seams_fix_width=64
        '''
        seams_fix_type=USDUSFMode.NONE
